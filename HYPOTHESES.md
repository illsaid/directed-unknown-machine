...
**Next test:** Test a supported sensitivity range entirely above a maximum threshold, verifying that the whole range can establish a violated gate without selecting one representative value.
...
**Evidence against:** The current repo has no real external tool under test except itself. Runs 5 through 35 produced their useful result by shaping, rejecting, repairing, preserving, bounding, clarifying, consistency-checking, evidence-binding, provenance-preserving, applicability-auditing, or sensitivity-bounding decision notes, not by offering a general failure explanation.
