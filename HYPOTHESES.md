...
**Next test:** Test a supported range that touches an inclusive threshold exactly, verifying that equality is judged from the supplied comparison rule rather than from an assumed strict or inclusive boundary.
...
**Evidence against:** The current repo has no real external tool under test except itself. Runs 5 through 36 produced their useful result by shaping, rejecting, repairing, preserving, bounding, clarifying, consistency-checking, evidence-binding, provenance-preserving, applicability-auditing, or sensitivity-bounding decision notes, not by offering a general failure explanation.
