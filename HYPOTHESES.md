...
**Next test:** Test two sources that disagree on the same measurement, verifying that disagreement remains explicit instead of being averaged, silently resolved, or converted into a false consensus.
...
**Evidence against:** The current repo has no real external tool under test except itself. Runs 5 through 25 produced their useful result by shaping, rejecting, repairing, preserving, bounding, clarifying, consistency-checking, evidence-binding, or provenance-preserving decision notes, not by offering a general failure explanation.
