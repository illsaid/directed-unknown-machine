...
**Next test:** Test a fully supplied adjustment method that uses a disputed assumption, verifying that transparent arithmetic does not turn an unsupported assumption into an observed fact.
...
**Evidence against:** The current repo has no real external tool under test except itself. Runs 5 through 32 produced their useful result by shaping, rejecting, repairing, preserving, bounding, clarifying, consistency-checking, evidence-binding, provenance-preserving, or applicability-auditing decision notes, not by offering a general failure explanation.
