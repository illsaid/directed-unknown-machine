...
**Next test:** Test an adjusted target-population estimate whose numeric result is supplied but whose adjustment method is incomplete, verifying that an opaque adjustment cannot resolve a gate.
...
**Evidence against:** The current repo has no real external tool under test except itself. Runs 5 through 31 produced their useful result by shaping, rejecting, repairing, preserving, bounding, clarifying, consistency-checking, evidence-binding, provenance-preserving, or applicability-auditing decision notes, not by offering a general failure explanation.
