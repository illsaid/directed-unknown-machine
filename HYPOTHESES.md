...
**Next test:** Test two independent constraints where one is satisfied and one is violated, verifying that the assignment reports each gate separately rather than collapsing the contract into one overall pass or fail.
...
**Evidence against:** The current repo has no real external tool under test except itself. Runs 5 through 21 produced their useful result by shaping, rejecting, repairing, preserving, bounding, clarifying, consistency-checking, or evidence-binding decision notes, not by offering a general failure explanation.
