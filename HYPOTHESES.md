...
**Next test:** Test threshold wording that supplies a number but no comparison operator, verifying that equality remains unresolved rather than inheriting an invented strict or inclusive rule.
...
**Evidence against:** The current repo has no real external tool under test except itself. Runs 5 through 40 produced their useful result by shaping, rejecting, repairing, preserving, bounding, clarifying, consistency-checking, evidence-binding, provenance-preserving, applicability-auditing, sensitivity-bounding, or threshold-resolving decision notes, not by offering a general failure explanation.
