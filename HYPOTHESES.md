...
- **Run 142 / `SCENARIOS/140-two-satisfied-fallbacks-no-precedence.md`:** Two independently conditioned fallback branches were fully satisfied, but Success supplied no precedence or tie-breaker. The existing non-nested branch obligation correctly preserved both as satisfied candidate branches while leaving governing authority unresolved rather than allowing an arbitrary choice. No executable change was justified.
- **Run 143 / `SCENARIOS/141-two-satisfied-fallbacks-explicit-precedence.md`:** The same two fully satisfied fallback branches supplied disposition policy DP-9 giving local reprocessing precedence. The existing obligation selected local reprocessing and preserved salvage shipment as satisfied but non-governing, but did not explicitly require unconditional precedence authority to be cited. The recommendation obligation now requires the applied precedence source to remain visible in the governing recommendation.
...
**Next test:** Supply explicit precedence whose cited policy conflicts with another supplied precedence source, testing whether the compiler refuses to choose without supplied applicability authority.
...
**Evidence against:** Runs 5–143 produced useful results by shaping and auditing decision contracts, not by providing general failure explanations.
