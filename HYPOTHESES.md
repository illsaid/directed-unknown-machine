...
**Next test:** Test the strict counterpart: a supported range whose upper bound equals a threshold stated as strictly below 500 milliseconds, verifying that equality violates the supplied rule.
...
**Evidence against:** The current repo has no real external tool under test except itself. Runs 5 through 37 produced their useful result by shaping, rejecting, repairing, preserving, bounding, clarifying, consistency-checking, evidence-binding, provenance-preserving, applicability-auditing, or sensitivity-bounding decision notes, not by offering a general failure explanation.
