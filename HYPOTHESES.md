...
**Next test:** Test a supplied assumption with an explicit sensitivity range that crosses the decision threshold, verifying that the brief reports the decision boundary rather than selecting a convenient point estimate.
...
**Evidence against:** The current repo has no real external tool under test except itself. Runs 5 through 33 produced their useful result by shaping, rejecting, repairing, preserving, bounding, clarifying, consistency-checking, evidence-binding, provenance-preserving, or applicability-auditing decision notes, not by offering a general failure explanation.
