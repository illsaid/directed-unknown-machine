...
**Next test:** Test a comparability reason supported only by a downstream interpretation rather than a direct supplied observation, checking whether the evidence boundary remains clear without sentence-level classification.
...
**Evidence against:** The current repo has no real external tool under test except itself. Runs 5 through 28 produced their useful result by shaping, rejecting, repairing, preserving, bounding, clarifying, consistency-checking, evidence-binding, or provenance-preserving decision notes, not by offering a general failure explanation.
