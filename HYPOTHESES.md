...
**Next test:** Test a constraint gate for which the supplied evidence is incomplete rather than clearly satisfying or violating it, verifying that the assignment leaves the gate unresolved without inventing a pass or blocker.
...
**Evidence against:** The current repo has no real external tool under test except itself. Runs 5 through 19 produced their useful result by shaping, rejecting, repairing, preserving, bounding, clarifying, or consistency-checking decision notes, not by offering a general failure explanation.
