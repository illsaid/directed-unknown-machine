...
REQUIREMENT_GROUPS = (
    ...
    (
        "Decision: recommend",
        (
            "Recommend one authorized action and name the governing Success branch. Connect every gate result to the branches that require it. A violated or unresolved gate blocks only branches that depend on it; it must not block a separate branch whose supplied conditions are fully satisfied. When Success explicitly supplies an action for a violated, unresolved, or conflicting condition, recommend that action only when that condition and every additional condition of the fallback branch are established, and cite the supplied evidence establishing the fallback trigger and every additional fallback-specific gate. If a fallback-specific gate is violated, report the fallback branch unavailable under the supplied evidence; if a fallback-specific gate is unresolved, report the branch awaiting evidence and identify the missing evidence. Do not request evidence for a gate already established as violated. Evaluate each supplied fallback branch independently: a violated or unresolved gate in one fallback does not block another fallback unless the second branch explicitly depends on it. Keep distinct fallback-specific gates independently evidenced: do not reuse support for one gate to satisfy another unless the supplied evidence explicitly supports both. A record that explicitly supports one gate but only implies another supports only the explicit gate; leave the implied gate unresolved. Evidence for a duration-bound gate governs only the duration it explicitly covers; preserve covered and uncovered periods separately, and leave the gate unresolved when the supplied duration is shorter than the branch requires. Complementary duration records may jointly satisfy a duration-bound gate only when their explicit scopes cover the entire required period without gaps or conflicting overlap; preserve each record and do not infer continuity outside the stated scopes. When explicit duration scopes overlap and conflict, preserve both results for the shared period, mark that period unresolved, and leave the full-duration gate unresolved even when every required period is nominally mentioned. Do not confuse a resolved evidence condition with satisfied branch authority. When every condition of a supplied branch is satisfied, recommend that branch without inventing additional gates. When one fully satisfied branch contains every condition of another plus additional satisfied conditions, recommend the more conditional supplied branch rather than arbitrarily choosing its weaker subset. When multiple fully satisfied branches are not nested and the supplied rule gives no precedence or tie-breaker, report branch authority unresolved and identify the missing precedence; do not choose among them. A conditional precedence rule governs only when every supplied condition activating it is established; if any activating condition is violated or unresolved, preserve the candidate branches and report precedence unresolved. Conflicting supplied evidence about an activating condition leaves conditional precedence unresolved unless supplied applicability evidence establishes which source governs; do not select a convenient source. When supplied applicability resolves conflicting trigger evidence, identify the governing source and preserve each displaced source as conflicting but non-governing rather than false or omitted. Applicability evidence governs only the scope it explicitly covers; when a trigger requires a broader period or population, preserve covered and uncovered scopes separately and leave the trigger unresolved unless every required scope is established. Do not infer an uncovered population from a covered population, even when the covered population is a supplied majority. When complementary governed scopes jointly cover every required population, evaluate a universal trigger across all scopes: it is satisfied only if every scope supports it and violated if any scope rejects it; do not average scope results. When governed population scopes overlap, apply only supplied overlap authority to the overlap before evaluating any broader trigger; preserve displaced source results as conflicting but non-governing within the overlap, and do not double-count the overlap. When conflicting authorities overlap and no supplied rule governs the shared scope, preserve that overlap as unresolved; do not inherit either source or resolve any broader trigger that depends on the overlap. When conditional precedence governs, cite the supplied evidence that establishes every activating condition. When supplied precedence selects among fully satisfied non-nested branches, recommend the governing branch, cite the supplied precedence that authorizes the selection, and preserve each displaced branch as satisfied but non-governing rather than failed or omitted. When supplied precedence authorities conflict, preserve every conflicting rule and leave branch authority unresolved unless supplied applicability or governance evidence establishes which precedence source governs; do not choose by field order, policy name, or convenience. When supplied governance resolves conflicting precedence authorities, cite both the governance rule and the selected precedence policy, preserve each displaced precedence policy as conflicting but non-governing, and preserve each displaced fully satisfied branch as satisfied but non-governing. A supplied governance rule governs only when the target is inside its stated scope and supplied evidence establishes that the target is outside every explicit exception. Preserve an applicable exception and leave the underlying precedence conflict unresolved rather than applying the rule's general authority to an excepted target. When exception applicability is unresolved, preserve that uncertainty, leave governance and branch authority unresolved, and name the evidence required to establish whether the exception applies; do not treat absence of exception evidence as evidence that the target is outside the exception. When supplied applicability authorities conflict about which evidence governs, preserve every authority and affected evidence source, leave the applicability result and every downstream authority unresolved, and name the governance evidence required to select among the applicability authorities; do not choose by source rank, specificity, recency, or convenience unless supplied authority authorizes that choice. Governance selecting an applicability authority governs only the exact status, metric, period, population, or other dimension it explicitly names; preserve any separate conflict outside that dimension as unresolved and do not borrow the governed result to settle it.",
        ),
    ),
)
