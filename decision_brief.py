...
REQUIREMENT_GROUPS = (
    ...
    (
        "Evidence provenance",
        (
            "Identify shared sources and the measurements each source actually supplies.",
            "When different sources overlap on the same measurement, disclose the overlap without counting it as support for a different measurement or as additional independent evidence beyond that shared measurement.",
    ...
    (
        ...
        (
            ...
            "When an adjusted or reweighted value is used to establish target-population applicability, preserve the original value and name the adjusted value, the supplied adjustment method, and the target population.",
            "An adjusted numeric result without a supplied auditable method cannot resolve a gate and must remain an unresolved claim.",
            "When an auditable adjustment method depends on an assumption, name the assumption and its supplied support, and keep the adjusted result conditional if that support is absent.",
        ),
    ),
    (
        "Sensitivity",
    ...
    (
        "Threshold conflict",
        (
            "When the same threshold is supplied with conflicting strict and inclusive comparison wording, preserve both rules and leave the gate unresolved unless the contract supplies an explicit precedence or reconciliation rule; do not select a rule based on field order.",
            "When the same metric is supplied with conflicting numeric thresholds, preserve every threshold value and leave the gate unresolved unless the contract supplies an explicit precedence or reconciliation rule.",
            ...
            "Do not silently choose the stricter threshold, the looser threshold, or the value appearing later.",
